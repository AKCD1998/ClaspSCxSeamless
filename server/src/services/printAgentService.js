...
const generatedFileRepository = require('../db/repositories/generatedFileRepository');
...
const { conflict } = require('../utils/apiError');
...
async function getPrintQueue() {
  ...
  const rows = await printJobRepository.listPrintQueueCandidates(env.autoPrintSince || null);
  const queue = [];

  for (const row of rows) {
    const record = processingRecordRepository.mapRecord(row);
...
async function createAgentPrintJob(body = {}) {
  ...
  try {
    ...
    const record = await processingRecordRepository.getProcessingRecordById(processingRecordId, client);
...
async function sendPrintEmailNotification(job, record) {
  if (!env.sendgridApiKey || !env.mailFrom || !env.docsRecipientEmail) {
    return {
      skipped: true,
      reason: 'SENDGRID_API_KEY, MAIL_FROM, or DOCS_RECIPIENT_EMAIL is not configured.',
    };
  }
...
async function completeAgentPrintJob(id) {
  ...
  const record = await processingRecordRepository.markPrinted(job.processingRecordId, 'auto-print-agent');

  let lineResult;
  try {
    lineResult = await sendPrintNotification(job, record);
  } catch (error) {
  ...
  let emailResult;
  try {
    emailResult = await sendPrintEmailNotification(job, record);
  } catch (error) {
  ...
  return { ok: true, job: finalJob, record, lineNotify: lineResult, emailNotify: emailResult };
}
