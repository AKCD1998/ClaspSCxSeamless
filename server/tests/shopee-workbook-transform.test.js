...
const { transformShopeeWorkbook } = require('../src/services/shopeeWorkbookTransform');
const { parseZipEntries, inflateEntry } = require('../src/services/xlsxDefaultFont');
...
test('9. applies exact comments, formats, fills, widths, heights, views, page setup', async () => {
  ...
  const master = result.workbook.getWorksheet('06');
  const profile = require('../src/services/shopeeAccountingCycles').MONTH_PROFILES['2026-06'];
...
test('guard: all 12 mapped source headers resolve to non-null column indexes', async () => {
  ...
  const { findShopeeHeader } = require('../src/services/shopeeWorkbookTransform');
